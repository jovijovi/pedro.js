...
test('GetDateByZone', () => {
	...
	log.RequestId().info("DateByZone(UTC)=", util.time.GetDateByZone('UTC'));
...
test('GetUnixTimestamp', () => {
	...
	log.RequestId().info("GetUnixTimestamp(UTC)=", util.time.GetUnixTimestamp(util.time.GetUnixTime(), 'UTC'));
